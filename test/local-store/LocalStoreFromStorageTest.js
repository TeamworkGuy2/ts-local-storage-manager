...
QUnit.test("local-store-from-storage-load-existing", function LocalStoreDefaultLoadExisting(sr) {
    var fullStoreHandler = new ClearFullStore(function (key) { return (key.length > 0 ? key.charCodeAt(0) << 16 : 0) + (key.length > 1 ? key.charCodeAt(1) : 0); });
    ...
    var wrapper1 = LocalStoreWrapper.newInst(baseStore, function (store, err) { return fullStoreHandler.clearOldItems(baseStore, true, err); }, true, true, undefined, true);
